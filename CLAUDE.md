...
- **The Draft gates ahead of prompts (SB-69):** while `state.draft` is set (`config.draftMode`, `core/draft.ts`), `reduce` refuses every action except `draftPick` and `concede` with reason `drafting`. That check runs before the pending-prompt gate and B20, and picks come from any player in any order.
...
- **Premoves (SB-68)** are local and speculative, rooms only. `src/net/premove.ts` is pure (the branch, the fold, the tracker, rollback and submission), `src/ui/usePremove.ts` wires it, and only real intents change shared state. A rollback's `reroll` is an ordinary active-player intent (`core/reroll.ts`), sent on the premover's own turn.
...
- Turn timer (SB-67): when it runs out, `useGame` answers this browser's own open prompt with the prompt's `defaultKeys`, then ends the turn, but only from the browser that controls the active seat (`src/ui/turntimer.ts`). `config.turnSeconds` already includes Time Flail, so never divide it again in the UI, and 0 means no timer. A room's lobby sets it (`LobbyPayload.timerOn`); hotseat reads `#hotseat:N:t<seconds>`. During a draft the clock is hidden, and an idle seat's next open slot is picked with its first option instead (SB-69).
- Mobile: `src/ui/links.ts` is the linked-card highlight store (`setLinkSource`, `useLinked`), `touch.ts` turns taps and long presses into card actions and previews, and `TapTip.tsx` gives `title` text a touch path. `mobile.css` holds the phone layout (max-width 700px) and is imported last in `main.tsx`, so it wins by source order.
- The opponents strip shows every player, you included. Your tile is `data-testid="seat-self"`, never `opponent` or `.opponent-turn`, which the e2e specs use to count and find opponents. The seat to move is pushed left with CSS `order`, not by reordering the DOM, so `getByTestId('opponent').first()` keeps its meaning. The anomaly chip opens `AnomalyPanel` over the board region; keep the chip's `title`, which e2e reads.

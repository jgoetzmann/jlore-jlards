/**
 * Binds the keymap in `keys.ts` to the window. Ported from the parked pass
 * (c67cf3f).
 *
 * The listener is attached once and reads the latest context and handler
 * through refs, so a table that re-renders on every view is not adding and
 * removing a window listener every time.
 *
 ...
 * invisible modality this exists to remove.
 */
...
export function useKeyboard(
  ...
  React.useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent): void => {
      ...
      if (!intent) return;
      // A keystroke we claimed must not also do its default thing.
      e.preventDefault();

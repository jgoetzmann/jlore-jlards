...
function compareForWin(
  state: GameState,
  scores: Record<PlayerId, number>,
  a: PlayerId,
  b: PlayerId,
): number {
  if (scores[a] !== scores[b]) return scores[a] > scores[b] ? 1 : -1;
  const ta = turnsTaken(state, a);
  const tb = turnsTaken(state, b);
  if (ta !== tb) return ta < tb ? 1 : -1;
  const da = deckIidsOf(state, a).length;
  const db = deckIidsOf(state, b).length;
  if (da !== db) return da < db ? 1 : -1;
  return 0;
}

/** SB-15 tiebreakers: most VP, then fewest turns taken, then smallest deck. */
export function winnersOf(state: GameState): PlayerId[] {
  const scores = scoreFinal(state);
  const alive = state.playerOrder.filter((id) => !state.players[id]?.eliminated);
  const pool = alive.length > 0 ? alive : [...state.playerOrder];
  if (pool.length === 0) return [];

  let best = pool[0];
  let bestSet: PlayerId[] = [best];
  for (const id of pool.slice(1)) {
    const cmp = compareForWin(state, scores, id, best);
    if (cmp > 0) {
      best = id;
      bestSet = [id];
    } else if (cmp === 0) {

...
/**
 * Fire every player's `gameEnd` queue, in seat order, at final scoring.
 * The queues are cleared as they fire.
 */
export function tickGameEnd(state: GameState): GameState {
  let next = state;
  for (const player of state.playerOrder) {
    const p = next.players[player];
    if (!p) continue;
    const due = p.delayed.filter((d) => d.when === 'gameEnd');
    if (due.length === 0) continue;
    const keep = p.delayed.filter((d) => d.when !== 'gameEnd');
    next = withPlayer(next, player, (pl) => ({ ...pl, delayed: keep }));
    for (const d of due) {
      next = resolveEffects(next, d.effects, {
        player,
        sourceIid: d.sourceIid ?? null,
    ...
    next = pushLog(next, 'delayedFired', player, { when: 'gameEnd', count: due.length });
  }
  return next;
}

/** How many effects are still queued on a player. Feeds SelfView.delayedCount. */
export function pendingDelayedCount(state: GameState, player: PlayerId): number {
  return state.players[player]?.delayed.length ?? 0;
}

/** Drop one queued effect by id without firing it. */
export function cancelDelayed(state: GameState, player: PlayerId, id: string): GameState {
  const p = state.players[player];
  if (!p) return state;
  if (!p.delayed.some((d) => d.id === id)) return state;
  return withPlayer(state, player, (pl) => ({ ...pl, delayed: pl.delayed.filter((d) => d.id !== id) }));
}
